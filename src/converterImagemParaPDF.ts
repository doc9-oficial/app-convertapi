...
export default async function converterImagemParaPDF(params: any): Promise<void> {
  try {
    ...
    const { file } = params;
    if (!file) {
      return console.log(docgo.result(false, null, 'Arquivo de imagem obrigatório'));
    }
    ...
    const result = await convertApi.convert('jpg', 'pdf', convParams);

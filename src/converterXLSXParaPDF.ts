...
export default async function converterXLSXParaPDF(params: any): Promise<void> {
  try {
    ...
    const { file } = params;
    if (!file) {
      return console.log(docgo.result(false, null, 'Arquivo XLSX obrigatório'));
    }
    ...
    const result = await convertApi.convert('xlsx', 'pdf', convParams);

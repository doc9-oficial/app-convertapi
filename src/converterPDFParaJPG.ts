...
export default async function converterPDFParaJPG(params: any): Promise<void> {
  try {
    ...
    const { file } = params;
    if (!file) {
      return console.log(docgo.result(false, null, 'Arquivo PDF obrigatório'));
    }
    ...
    const result = await convertApi.convert('pdf', 'jpg', convParams);

...
export default async function converterPPTXParaPDF(params: any): Promise<void> {
  try {
    ...
    const { file } = params;
    if (!file) {
      return console.log(docgo.result(false, null, 'Arquivo PPTX obrigatório'));
    }
    ...
    const result = await convertApi.convert('pptx', 'pdf', convParams);
